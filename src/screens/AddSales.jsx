...
const AddSales = () => {
    ...
    const getDebitors = async () => {
        try {
            const snap = await firestore().collection("account").where("group", "==", "debitor").get();
    ...
    const addCategory = async () => {
        try {
            ...
            ToastAndroid.showWithGravity('Sale Added Successfully!', ToastAndroid.SHORT, ToastAndroid.TOP);
            setLoader(false);
        } catch (error) {
            setLoader(false);
    ...
    const handleSelectItem = (optionsItem) => {
        ...
        <SafeAreaView style={{ flex: 1 }}>
            <ScrollView contentContainerStyle={styles.scrollViewContent}>
                <View style={{ flex: 1, backgroundColor: colors.primary }}>
                    ...
                    {itemOptions &&
                        <SearchDropdown options={itemOptions} onSelect={handleSelectItem} />
                    }
